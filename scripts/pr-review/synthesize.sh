...
CHAIR_PRIMARY_MODEL="${CHAIR_PRIMARY_MODEL:-global.anthropic.claude-fable-5-1}"
...
# Kiro 무툴 계약 위반(run-panel.sh 의 kiro-agent-fallback.flag) — 러너의 kiro-cli 가
# `--agent pr-review-notools` 를 무시하고 툴 있는 기본 에이전트로 실행했거나(폴백 시그니처),
# 셀 출력에 tool-use 실행 흔적(`(using tool: …)`)이 있었다. 응답은 이미 폐기됐고
# coverage-severe 로 강제 FAIL 되지만, "왜 FAIL 인지"를 코멘트에서 바로 읽게 한다.
if [ -s "$WORK/kiro-agent-fallback.flag" ]; then
  AGENTFAIL_DETAIL="$(tr -d '`' < "$WORK/kiro-agent-fallback.flag" | tr '\n' ' ' | sed 's/ *$//')"
  { echo "🔓 **Kiro 무툴 계약 위반**: kiro-cli 가 \`--agent pr-review-notools\` 를 무시하고 툴 있는 기본 에이전트로 실행했거나 셀 안에서 툴이 실행됨 (\`$AGENTFAIL_DETAIL\`) — 해당 셀 응답은 폐기, 강제 FAIL. 러너 이미지의 kiro-cli 버전/에이전트 스키마 변경 여부 확인 필요(docs/runbooks/pr-review-panel.md)."
    echo ""
    cat "$OUT"
  } > "$OUT.tmp" && mv "$OUT.tmp" "$OUT"
fi
